...
void HB_CK_Start(void) 
{
    ...
    HB_CK_CLKEN |= HB_CK_CLKEN_MASK;
	HB_CK_CLKSTBY |= HB_CK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: HB_CK_Stop
********************************************************************************
*
* Summary:
...
void HB_CK_Stop(void) 
{
    ...
    HB_CK_CLKEN &= (uint8)(~HB_CK_CLKEN_MASK);
	HB_CK_CLKSTBY &= (uint8)(~HB_CK_CLKSTBY_MASK);
}
...
{
    if ((HB_CK_CLKEN & HB_CK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = HB_CK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = HB_CK_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(HB_CK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(HB_CK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: HB_CK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: HB_CK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = HB_CK_GetSourceRegister();
    uint16 oldDivider = HB_CK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = HB_CK_CLKEN & HB_CK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((HB_CK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: HB_CK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(HB_CK_DIV_PTR);
}


/*******************************************************************************
* Function Name: HB_CK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    HB_CK_MOD_SRC |= modeBitMask & (uint8)HB_CK_MODE_MASK;
}


/*******************************************************************************
* Function Name: HB_CK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    HB_CK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(HB_CK_MODE_MASK));
}


/*******************************************************************************
* Function Name: HB_CK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return HB_CK_MOD_SRC & (uint8)(HB_CK_MODE_MASK);
}


/*******************************************************************************
* Function Name: HB_CK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = HB_CK_GetDividerRegister();
    uint8 oldSrc = HB_CK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        HB_CK_MOD_SRC |= CYCLK_SSS;
        HB_CK_MOD_SRC =
            (HB_CK_MOD_SRC & (uint8)(~HB_CK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: HB_CK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return HB_CK_MOD_SRC & HB_CK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: HB_CK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    HB_CK_PHASE = clkPhase & HB_CK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: HB_CK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return HB_CK_PHASE & HB_CK_PHASE_MASK;
}
